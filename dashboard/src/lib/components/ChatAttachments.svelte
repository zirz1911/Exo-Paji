...
{#if files.length > 0}
  <div class="flex flex-wrap gap-2 mb-3 px-1">
    {#each files as file (file.id)}
      <div
        class="group relative flex items-center gap-2 bg-exo-dark-gray/80 border border-exo-green/30 rounded px-2.5 py-1.5 text-xs font-mono transition-all hover:border-exo-green/50 hover:shadow-[0_0_10px_rgba(0,255,65,0.1)]"
      >
        ...
        {#if file.preview && getFileCategory(file.type, file.name) === "image"}
          <img
            src={file.preview}
            alt={file.name}
            class="w-8 h-8 object-cover rounded border border-exo-green/20"
          />
        {:else}
        ...
        <div class="flex flex-col min-w-0">
          <span
            class="text-exo-green truncate max-w-[120px]"
            title={file.name}
          >
